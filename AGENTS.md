...
- **Session summarization uses wrong ID for API sessions** — `summarize_session` queries observations via `content_session_id` (IDE ID) instead of `session_id` (UUID). API-created sessions have different values → empty summaries. File: `crates/service/src/session_service.rs`
- **PG save_observation title dedup mismatch** — PG `save_observation` throws error on duplicate titles (`idx_obs_title_norm` unique constraint) instead of returning `Ok(false)` like SQLite. Causes 500 errors. File: `crates/storage/src/pg_storage/observations.rs`
- **AppState semaphore ignored** — `process_pending_queue` and `start_background_processor` create local semaphores, ignoring global `AppState.semaphore`. Unbounded concurrency under load. Files: `crates/http/src/handlers/queue.rs`, `queue_processor.rs`
