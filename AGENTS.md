...
- **Dedup sweep overwrite bug** — In `run_dedup_sweep`, the keeper and duplicate are chosen based on `noise_level`, but `merge_into_existing` always passes the duplicate as the `newer` parameter. Because `compute_merge` blindly prefers the title and type of the `newer` parameter, the more critical keeper's metadata is overwritten by the less critical duplicate's metadata.
- **Infinite memory time hierarchy violation** — The compression pipeline batches up to 100 raw events into a `summaries_5min` record without restricting the actual time span. If 100 events span three days, it creates a single '5min' summary for those three days, destroying the temporal deep zoom structure.
- **SessionService dead code** — `SessionService::summarize_session_from_export` executes an external `opencode` CLI binary, but is entirely unused by any HTTP handler, MCP tool, or internal job.
- **QueueProcessor UUID format violation** — Deterministic message IDs in the queue processor are generated by slicing SHA-256 bytes into a `uuid::Builder::from_bytes` call without setting the RFC 4122 version (v5) and variant bits, generating a non-compliant UUID.
- **Dedup missing title and type updates** — Context-aware updates silently drop title and observation_type changes in `PgStorage::merge_into_existing` which entirely omits `title` and `observation_type` in its SQL UPDATE query.
- **Queueprocessor bottleneck** — The background queue processor blocks while awaiting compression tasks to complete, resulting in head-of-line blocking that limits overall pipeline concurrency.
- **Privacy leak in title** — Explicit `save_memory` API inputs apply privacy filtering strictly to the message body but fail to sanitize custom manual titles inputted directly via the HTTP layer.
- **Privacy filter omission** — Tool calls generated via backend background queues completely bypass privacy and injection string sanitization layers when persisted dynamically via session_observations handlers.
- **Search endpoint abstraction bypass** — Primary semantic searching filters directly skip vector abstraction interfaces resulting in raw textual search degradation anytime basic properties (like dates and projects) are supplied.
- **Settings state ignored** — Changes made by HTTP endpoints directly updating `AppState` attributes fail to sync down to instantiated backend clients running out of initialization config env bindings.
